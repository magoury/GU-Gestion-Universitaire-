...
import { MoneyIcon, BuildingIcon, ClockIcon, TeachersIcon, RefreshIcon } from '../../components/icons/Icons.jsx';

// Fonction de hachage de couleur déterministe pour l'avatar
...
function SuperAdminDashboard() {
  ...
  const [kpis, setKpis] = useState({ totalUniversites: 0, nbUniversitesActives: 0, mrr: 0, nbTotalEtudiants: 0, nbAlertes: 0 });
  const [revenusMensuels, setRevenusMensuels] = useState([]);
  const [loading, setLoading] = useState(true);

  // Filtres
  const [recherche, setRecherche] = useState('');
  const [filtrePlan, setFiltrePlan] = useState('all');
  const [filtreStatut, setFiltreStatut] = useState('all');

  // Tiroir de détails (Drawer)
  const [universiteDetails, setUniversiteDetails] = useState(null);
  const [logsDetails, setLogsDetails] = useState([]);
  const [loadingLogs, setLoadingLogs] = useState(false);

  // Modal de confirmation suspension
  const [univAActionner, setUnivAActionner] = useState(null); // { id, nom, suspendre }

  // Charger les données de la plateforme
  const chargerDonnees = async () => {
    setLoading(true);
    try {
      const uList = await listerUniversites();
      const stats = await lireKPIsGlobaux();
      const revs = await lireRevenusMensuels();
      setUniversites(uList);
      setKpis(stats);
      setRevenusMensuels(revs);
    } catch (err) {
      console.error('Erreur de chargement des données Super Admin:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Charger les logs d'audit du drawer au besoin
  const ouvrirDrawerDetails = async (univ) => {
  ...
  // Actionner la suspension/réactivation
  const executerActionUniv = async () => {
    ...
    try {
      ...
      // Mettre à jour le drawer s'il est ouvert pour la même université
      if (universiteDetails && universiteDetails.id === univAActionner.id) {
        const misAJour = { ...universiteDetails, statut: univAActionner.suspendre ? 'suspendu' : 'actif' };
        setUniversiteDetails(misAJour);
        const logs = await lireAuditLogs(univAActionner.id, { limite: 15 });
        setLogsDetails(logs);
      }
    } catch (err) {
      console.error('Erreur action université:', err);
      alert('Erreur lors de la mise à jour du statut.');
    }
  };

  // Liste des universités filtrées
  const univsFiltrees = useMemo(() => {
  ...
  // Valeur maximale pour le graphe de croissance
  ...
  return (
    <div className="w-screen h-screen overflow-hidden flex font-body text-xs text-on-surface bg-bg select-none">
            ...
            <>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-on-surface-muted">Vue d'ensemble en temps réel de tous les tenants et de l'état financier de la plateforme.</p>
                    </div>
                    <button
                      onClick={chargerDonnees}
                      title="Rafraîchir"
                      className="btn btn-sm btn-ghost h-8 min-h-[32px] w-8 p-0 flex items-center justify-center rounded-md border border-white/10"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-4 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xl font-bold font-display text-accent mt-1">
                          {kpis.mrr.toLocaleString('fr-FR')} FCFA
                        </div>
                        <span className="text-[9px] text-success font-medium block mt-0.5">+12.5% this quarter</span>
                      </div>
                    ...
                    <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-4 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xl font-bold font-display text-on-surface mt-1">
                          {kpis.nbUniversitesActives} / {kpis.totalUniversites}
                        </div>
                        <span className="text-[9px] text-accent font-medium block mt-0.5">+1 new this month</span>
                      </div>
                      <div className="w-9 h-9 rounded-md bg-surface-high border border-white/5 flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-4 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xl font-bold font-display text-on-surface mt-1">99.99%</div>
                        <span className="text-[9px] text-on-surface-muted/50 block mt-0.5">Global Infrastructure</span>
                      </div>
                      <div className="w-9 h-9 rounded-md bg-surface-high border border-white/5 flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-4 flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xl font-bold font-display text-on-surface mt-1">
                          {kpis.nbTotalEtudiants.toLocaleString('fr-FR')}
                        </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-1">
                                ...
                                <tr key={univ.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                  <td className="py-2.5 pl-0 flex items-center gap-2">
                                    <div
                                      ...
                                      className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white border border-white/10 flex-shrink-0"
                                    >
                                      {initiales}
                                    </div>
                                  ...
                                  <td className="py-2.5">
                                    <span className="badge badge-xs badge-outline border-accent text-accent font-medium">{univ.plan}</span>
                                  </td>
                                  <td className="py-2.5">
                                  ...
                                  <td className="py-2.5 text-right">
                                    <div className="flex items-center justify-end gap-1.5">
                                      <button
                                        onClick={() => navigate(`/superadmin/university/${univ.id}`)}
                                        className="btn btn-xs h-6 min-h-[24px] px-2 bg-primary/20 hover:bg-primary text-primary-container border border-primary/20 hover:text-white"
                                      >
                                        Gérer
                                      </button>
                                      <button
                                        onClick={() => setUnivAActionner({ id: univ.id, nom: univ.nom, suspendre: univ.statut !== 'suspendu' })}
                                        className={`btn btn-xs h-6 min-h-[24px] px-2 border border-white/5 ${
                                          univ.statut === 'suspendu'
                                            ? 'bg-success/20 hover:bg-success text-success-content'
                                            : 'bg-error/20 hover:bg-error text-error-content'
                                        }`}
                                      >
                                        {univ.statut === 'suspendu' ? 'Réactiver' : 'Suspendre'}
                                      </button>
                                      <button
                                        onClick={() => ouvrirDrawerDetails(univ)}
                                        className="btn btn-xs h-6 min-h-[24px] px-2 btn-ghost border border-white/10"
                                      >
                                        Détails
                                      </button>
                                    </div>
                    ...
                    {/* Colonne droite (Graphique de croissance) */}
                    <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-4 flex flex-col justify-between">
                      <div className="border-b border-white/10 pb-2 mb-2">
                        ...
                        <p className="text-[9px] text-on-surface-muted">Revenus mensuels récurrents cumulés.</p>
                      </div>

                      {/* Graphe en barres CSS pur */}
                            ...
                            <div key={r.mois} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                              ...
                              <div
                                ...
                                className={`w-full rounded-t-sm transition-all duration-500 hover:brightness-125 relative group ${
                                  estDernier ? 'bg-accent' : 'bg-primary-container'
                                }`}
                              >
                                {/* Tooltip */}
                                <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-surface border border-white/15 px-1.5 py-0.5 rounded text-[8px] font-semibold text-accent opacity-0 group-hover:opacity-100 transition-opacity z-10 whitespace-nowrap">
                      ...
                      <div className="flex items-center justify-between border-t border-white/10 pt-2.5 mt-2 text-[10px]">
                        <div>
                          ...
                          <span className="text-success font-bold block">+24.8%</span>
                        </div>
                        <div className="text-right">
                          <span className="text-on-surface-muted">Tenant Churn</span>
                          <span className="text-on-surface-muted/50 font-bold block">0.4%</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-4 mt-2">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-white/10 pb-3 mb-4">
                      <div>
                        <h3 className="font-semibold text-sm text-on-surface">Toutes les universités clientes</h3>
                        <p className="text-[9px] text-on-surface-muted">Rechercher et filtrer les tenants académiques.</p>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <input
                          ...
                          className="h-8 px-3 rounded bg-white/5 border border-white/10 text-[10px] text-on-surface placeholder-on-surface-muted/50 focus:outline-none focus:border-accent"
                        />
                        ...
                        <select
                          ...
                          className="h-8 px-2 rounded bg-surface border border-white/10 text-[10px] text-on-surface focus:outline-none"
                        >
                    ...
                    {univsFiltrees.length === 0 ? (
                      <div className="p-8 text-center text-on-surface-muted">
                        Aucune université ne correspond aux critères de recherche.
                      </div>
                    ) : (
                        ...
                        {univsFiltrees.map((univ) => {
                          ...
                          return (
                            <div key={univ.id} className="bg-surface/80 border border-white/10 rounded-lg p-4 flex flex-col justify-between gap-3 hover:border-white/20 transition-all">
                              ...
                              {/* Haut card (Avatar + Nom + Plan) */}
                              <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-2.5 overflow-hidden">
                                  <div
                                    style={{ backgroundColor: color }}
                                    className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white border border-white/10 flex-shrink-0"
                                  >
                                    {initiales}
                                  </div>
                                ...
                                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                                  {/* Plan Badge */}
                                  <span className={`badge badge-xs badge-outline uppercase tracking-wider font-semibold text-[8px] ${
                                    univ.plan.toLowerCase() === 'premium'
                                      ? 'border-accent text-accent'
                                      : univ.plan.toLowerCase() === 'enterprise'
                              ...
                              {/* Milieu card (Infos étudiants & expiration) */}
                              <div className="grid grid-cols-2 gap-2 bg-white/2 p-2 rounded border border-white/5 text-[10px]">
                                <div>
                                  ...
                                  <span className="font-semibold">{univ.nbEtudiants} étudiants</span>
                                </div>
                                <div>
                                  <span className="text-on-surface-muted block text-[9px]">Date d'expiration</span>
                                  <span className="font-semibold">
                                    {new Date(univ.dateExpiration).toLocaleDateString('fr-FR')}
                                  </span>
                                </div>
                              </div>

                              {/* Bas card (Boutons d'action) */}
                              <div className="grid grid-cols-3 gap-1.5 mt-1">
                                <button
                                  onClick={() => navigate(`/superadmin/university/${univ.id}`)}
                                  className="btn btn-xs h-7 min-h-[28px] bg-primary hover:bg-primary-container text-white border-none font-semibold uppercase text-[9px] tracking-wider rounded"
                                >
                                  Gérer
                                </button>
                                <button
                                  onClick={() => setUnivAActionner({ id: univ.id, nom: univ.nom, suspendre: univ.statut !== 'suspendu' })}
                                  className={`btn btn-xs h-7 min-h-[28px] border border-white/5 font-semibold uppercase text-[9px] tracking-wider rounded ${
                                    univ.statut === 'suspendu'
                                      ? 'bg-success/20 hover:bg-success text-success-content'
                                      : 'bg-error/20 hover:bg-error text-error-content'
                                  }`}
                                >
                                  {univ.statut === 'suspendu' ? 'Activer' : 'Suspendre'}
                                </button>
                                <button
                                  onClick={() => ouvrirDrawerDetails(univ)}
                                  className="btn btn-xs h-7 min-h-[28px] btn-ghost border border-white/10 font-semibold uppercase text-[9px] tracking-wider rounded"
                                >
                                  Détails
                                </button>
                              </div>
              ...
              {activeSection !== 'overview' && (
                <div className="bg-surface/60 backdrop-blur-sm border border-white/10 rounded-lg p-6 text-center text-on-surface-muted">
                  ...
                  <p className="text-[10px]">Cette section du Command Center (Super Admin) est rattachée au monitoring global.</p>
                </div>
              )}
            </>
          )}
      ...
      {/* Drawer de détails latéral (Drawer fixe à droite) */}
      {universiteDetails && (
          ...
          <div className="w-80 h-screen bg-surface border-l border-white/10 p-5 flex flex-col justify-between z-50 animate-slide-left">
            <div>
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-4">
                <div className="flex flex-col">
                  <h3 className="font-semibold text-sm text-accent truncate max-w-[200px]" title={universiteDetails.nom}>
                    {universiteDetails.nom}
                  </h3>
                  <span className="text-[9px] text-on-surface-muted">Dossier Infrastructure</span>
                </div>
                <button
                  onClick={() => setUniversiteDetails(null)}
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex justify-between border-b border-white/5 py-1">
                  ...
                  <span className="font-semibold">
                    {new Date(universiteDetails.dateExpiration).toLocaleDateString('fr-FR')}
                  </span>
                </div>
                <div className="flex justify-between border-b border-white/5 py-1">
              ...
              <div className="mt-6">
                <div className="text-[10px] font-semibold text-on-surface-muted uppercase tracking-wider mb-2">
                ...
                {loadingLogs ? (
                  ...
                  <div className="text-[10px] text-on-surface-muted italic">Aucune activité enregistrée sur ce tenant.</div>
                ) : (
                    ...
                    {logsDetails.map((log) => (
                      <div key={log.id} className="p-2 bg-white/2 rounded border border-white/5 flex flex-col gap-1 text-[9px] hover:bg-white/5 transition-all">
                        <div className="flex items-center justify-between text-accent font-semibold">
                          ...
                          <span className="text-on-surface-muted/50 font-normal">
            ...
            {/* Impersonnalisation directe */}
            <div className="mt-4 pt-4 border-t border-white/10">
              <button
                onClick={() => navigate(`/superadmin/university/${universiteDetails.id}`)}
                className="w-full btn btn-sm bg-accent hover:bg-accent/80 text-bg border-none font-semibold text-xs rounded uppercase tracking-wider py-2 h-auto"
              >
                Impersonnaliser la session &rarr;
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-surface border border-white/10 rounded-lg p-5 max-w-sm w-full flex flex-col gap-4 animate-scale-up">
            <div>
              <h3 className="font-semibold text-sm text-on-surface">
                {univAActionner.suspendre ? 'Suspendre l\'université ?' : 'Réactiver l\'université ?'}
              </h3>
              <p className="text-[10px] text-on-surface-muted mt-1 leading-normal">
                {univAActionner.suspendre
                  ? `Voulez-vous suspendre l'université "${univAActionner.nom}" ? Cette action désactivera l'accès à la plateforme pour tous les utilisateurs (étudiants, enseignants, administrateurs) de cet établissement.`
                  : `Voulez-vous réactiver l'université "${univAActionner.nom}" ? Cela restaurera instantanément les accès à l'espace de formation.`}
              </p>
            </div>
            <div className="flex items-center justify-end gap-2">
              <button
                onClick={() => setUnivAActionner(null)}
                className="btn btn-sm btn-ghost h-8 min-h-[32px] px-3 font-semibold text-xs border border-white/10 rounded"
              >
                Annuler
              </button>
              <button
                onClick={executerActionUniv}
                className={`btn btn-sm h-8 min-h-[32px] px-3 font-semibold text-xs border-none rounded ${
                  univAActionner.suspendre ? 'bg-error hover:bg-error/80 text-white' : 'bg-success hover:bg-success/80 text-white'
                }`}
              >
                {univAActionner.suspendre ? 'Suspendre' : 'Réactiver'}
              </button>
            </div>
          </div>
        </div>
